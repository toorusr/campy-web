...
<div class="p-8 bg-purple @if ($page->active === 'no') border-t-4 border-purple @endif">
    ...
    @endif
    <h1 class="text-white">
        Komm' zum {{ $page->type }} in {{ $page->city }}
        @if ($page->days === 1)
            ({{ date('d.m.', $page->date_start) }})
        @else
            ({{ date('d.m.', $page->date_start) }} bis {{ date('d.m.y', $page->date_end) }})
        @endif
    </h1>
    ...
    <div class="md:flex mt-4">
        <div class="md:w-2/3 bg-white rounded p-4">
            ...
            @component('_components.camps.facts')
                ...
                @slot('locationlink') {{ $page->locationlink }} @endslot
                @slot('timestart') {{ $page->time_start }} @endslot
                @slot('timeend') {{ $page->time_end }} @endslot
                @slot('from') {{ date('d.m.', $page->date_start) }} @endslot
                @slot('to') {{ date('d.m.y', $page->date_end) }} @endslot
                @slot('days') {{ $page->days }} @endslot
                @slot('cost') {{ $page->cost }} @endslot
                @slot('costlaptop') {{ $page->costlaptop }} @endslot
                @slot('meals') {{ $page->meals }} @endslot
                @slot('type') {{ $page->type }} @endslot
                @slot('laptopfree') {{ $page->laptopfree }}@endslot
            @endcomponent
